...
modules.define('image', function (provide) {
    ...
    var _moduleName = this.name;

    function getDataPath(imageNode) {

        var data = imageNode.dataset.block;
    ...
    provide(function (addQuerySelector) {
        var imgList = document.querySelectorAll('.' + _moduleName + addQuerySelector),
            promiseImg = void 0;

        [].map.call(imgList, function (item, i) {
            promiseImg = loadImage(getDataPath(item));

            promiseImg.then(function (res) {
                item.src = res;
            }).catch(function (err) {
                console.error(err);
            });
